...
from ..utils import comparable, is_reserved_or_static, strip_reconciler_marker
...
class ComponentTemplateReconciler(BaseReconciler):
    kind = "component_templates"

    def list_actual(self, api: OpenSearchAPI) -> Dict[str, Dict[str, Any]]:
        data = api.get_json("/_component_template")
    ...
    def get_actual(self, api: OpenSearchAPI, obj: ManagedObject) -> Optional[Dict[str, Any]]:
        if api.head(f"/_component_template/{obj.name}") == 404:
            return None
        data = api.get_json(f"/_component_template/{obj.name}")
        items = data.get("component_templates", [])
        if not items:
            return None
        return items[0].get("component_template")
